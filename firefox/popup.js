...
async function getStorage(element, nonCheckboxes = false) {
  ...
  if (!nonCheckboxes) {
    ...
    const checkedRadioElement = document.getElementById(checkedRadioID['labels'])
...
checkboxes.forEach(checkbox => checkbox.addEventListener('change', setStorageCheckbox))
